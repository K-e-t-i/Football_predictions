...
def main():
    ...
    # import results 2 last team matches
    # Team_Matches = [list_of_home_team_scores, list_of_away_team_scores]
    Teams_Matches = [0, 0]
    for j in range(2):
        Matches = []
    ...
    # create a list of teams to make a ranking
    Teams = []
    ...
    # create extra efficiency based on scores
    home_efficiency = 0
    away_efficiency = 0

    # add or sub efficiency depending on the position in the ranking
    for i in range(len(Sorted_Ranking)):
        if Sorted_Ranking[i][0] == Teams_to_predict[0]:
            home_efficiency = 0.2 - i * 0.02

        elif Sorted_Ranking[i][0] == Teams_to_predict[1]:
            away_efficiency = 0.2 - i * 0.02
    ...
    # Sum up the efficiency of teams
    ...
    # Subtracting efficiency
    sub = abs(good_eff_A - good_eff_H)

    # Show result depending on the efficiency
    if sub > 0.08:
